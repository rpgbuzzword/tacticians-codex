...
const createBuild = async (req, res) => {
  ...
  try {
    ...
    const buildResult = await client.query(
      'INSERT INTO builds (title, notes, is_public) VALUES ($1, $2, $3) RETURNING id',
      [title, notes, is_public]
    );
    ...
    for (const unit of army) {
      ...
      await client.query(
        `INSERT INTO units (
          build_id, name, base_class, promoted_class, notes,
          inventory, weapon_mastery, supports,
...
const updateBuild = async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE builds SET title = $1, notes = $2, is_public = $3 WHERE id = $4',
      [title, notes, is_public, buildId]
    );
